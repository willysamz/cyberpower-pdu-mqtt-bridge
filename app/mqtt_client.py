...
class MqttClient:
    ...
    def __init__(
        ...
        client_id: str = "cyberpower-pdu-mqtt-bridge",
        keepalive: int = 60,
        qos: int = 0,
        availability_topic: str = "pdu/bridge/available",
    ) -> None:
        self.host = host
        self.port = port
    ...
    def _new_client(self) -> aiomqtt.Client:
        # Last will: publish `offline` to the availability topic when the
        # bridge disconnects without an explicit cleanup (network blip etc.).
        will = aiomqtt.Will(
    ...
    async def session(self) -> AsyncIterator[MqttClient]:
        """Open the MQTT connection for the lifetime of the bridge.

        Use:
            async with mqtt.session():
                await mqtt.publish(...)
        """
        async with self._new_client() as client:
            ...
            try:
                ...
                # Best-effort: announce offline on a clean shutdown too. Errors
                # here are swallowed because the connection may already be gone.
                try:
                    await client.publish(
